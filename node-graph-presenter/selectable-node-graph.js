...
AFRAME.registerComponent('selectable-node-graph', {
	...
	init: function () {
		...
		const spinner = this.spinner = document.createElement('a-gltf-model');
		...
		this.graphWorker = new Worker("../node-graph-presenter/build/graphWorker.js",
		...
		this.graphWorker.onmessage = event => {
			switch (event.data.kind) {
				...
				case 'UPDATE':   // upserts nodes and edges
					...
					for (const node of event.data.nodes) {
						...
						if (isNew) {
							...
							this.el.appendChild(el);   // is an asynchronous operation
						}
					}
					for (const link of event.data.links) {
						if (!link.id) { console.warn(`edge from ${link.source.title} to ${link.target.title} lacks ID`); }
						let edgeEl = document.getElementById(link.id);
						const isNew = !edgeEl;
						if (isNew) {
							edgeEl = document.createElement('a-entity');
				...
				case 'LOAD_SUCCESS':
					try {
						console.info(`LOAD_SUCCESS; adjusting spread & emitting graph-loaded`, event.data.msg);
						this.adjustSpread();
						this.el.emit('graph-loaded');
						this.spinner?.removeState(STATE_SPINNING);
					} catch (err) {
					...
					break;
				case 'LOAD_FAIL':
					console.error(`load failed`, event.data.msg);
					this.spinner?.removeState(STATE_SPINNING);
					break;
		...
		this.graphWorker.onerror = errEvent => {
			console.error(`graphWorker error “${errEvent.message}” ${errEvent.filename} line ${errEvent.lineno} column ${errEvent.colno}`);
			postMessage({kind: 'TRANSIENT_MSG', msg: `Is there a different file with this data?`});
		}
		this.graphWorker.onmessageerror = messageEvent => {
			console.error(`graphWorker error id: ${messageEvent.lastEventId} origin: ${messageEvent.origin} source: ${messageEvent.source} data: ${messageEvent.data}`);
			postMessage({kind: 'TRANSIENT_MSG', msg: `Reload this page & report what you were doing when this error occurred`});
		}
	},
	...
	openUrl: function (evt) {
		...
		if (['https:', 'http:', 'ftp:', 'ftps:', 'sftp:', 'file:', 'data:', 'blob:'].includes(url?.protocol)) {
			this.spinner?.addState(STATE_SPINNING);
	...
	update: async function (oldData = {}) {
		try {
			...
			if (this.data.spreadHoriz > 0 && this.data.spreadHoriz < Infinity &&
				...
				for (const graphEl of Array.from(this.el.children)) {
					const graphElNodeAttr = graphEl.getAttribute('graph-node');
					if (graphElNodeAttr) {   // avoids setAttribute, which calls Croquet
						graphEl.object3D.position.x = graphElNodeAttr.naturalPosition.x * this.data.spreadHoriz;
						graphEl.object3D.position.z = graphElNodeAttr.naturalPosition.z * this.data.spreadHoriz;
	...
	adjustSpread: function () {
		...
		console.log(`adjustSpread spreadHoriz=${spreadHoriz} spreadVert=${spreadVert} bbSize=`, bbSize, "boundingBox=", boundingBox);
